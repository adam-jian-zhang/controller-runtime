...
graph TB
    Webhooks[Webhooks]
    Webhooks --> Admission[Admission Webhooks]
    Webhooks --> Conversion[Conversion Webhooks]
    Webhooks --> Authentication[Authentication Webhooks]
    
    Admission --> Mutating[Mutating Webhooks]
    Admission --> Validating[Validating Webhooks]
    
    Mutating --> Default[Default Values]
    Mutating --> Transform[Transform Objects]
    
    Validating --> Validate[Validate Objects]
    Validating --> Reject[Reject Invalid]
    
    style Webhooks fill:#e1f5ff
    style Admission fill:#fff4e1
    style Conversion fill:#f0f0f0
```
...
sequenceDiagram
    ...
    Webhook->>Handler: Decode & Route
    
    alt Mutating Webhook
        Handler->>Handler: Default()
        Handler->>Webhook: Patches
    else Validating Webhook
        Handler->>Handler: ValidateCreate/Update/Delete()
        Handler->>Webhook: Allowed/Denied
    end
...
- [Manager Package](./01-manager.md) - Manages webhook server
- [Builder Package](./04-builder.md) - Builds webhooks
- [Client Package](./05-client.md) - Used in webhook handlers
