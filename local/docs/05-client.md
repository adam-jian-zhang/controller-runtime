...
graph TB
    ...
    Client --> Reader[Reader Interface]
    Client --> Writer[Writer Interface]
    Client --> StatusClient[StatusClient Interface]
    ...
    Writer --> DeleteAllOf[DeleteAllOf]
    
    StatusClient --> StatusWriter[Status SubResource]
...
graph LR
    Client[Split Client]
    Client -->|Read| Cache[Cache]
    Client -->|Write| API[API Server]
...
- [Cache Package](./06-cache.md) - Provides cached reads
- [Manager Package](./01-manager.md) - Provides client instances
- [Reconcile Package](./03-reconcile.md) - Uses client for reconciliation
