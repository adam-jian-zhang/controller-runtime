...
graph TB
    Manager[Manager]
    Manager --> Cluster[Cluster Interface]
    Manager --> Lifecycle[Lifecycle Management]
    Manager --> HealthChecks[Health Checks]
    Manager --> Metrics[Metrics Server]
    Manager --> LeaderElection[Leader Election]
    Manager --> WebhookServer[Webhook Server]
    
    Cluster --> Client[Client]
    Cluster --> Cache[Cache]
    Cluster --> Scheme[Scheme]
    Cluster --> RESTMapper[RESTMapper]
    
    style Manager fill:#e1f5ff
    style Lifecycle fill:#fff4e1
    style HealthChecks fill:#f0f0f0
```
...
graph TB
    ...
    Runnable --> Controller[Controllers]
    Runnable --> Webhook[Webhooks]
    Runnable --> Custom[Custom Runnables]
    
    Controller --> LeaderElection1[Leader Election Mode]
    Webhook --> LeaderElection2[Leader Election Mode]
    Custom --> LeaderElection3[Leader Election Mode]
    
    LeaderElection1 --> Always[Always Running]
    LeaderElection1 --> OnlyLeader[Only When Leader]
...
sequenceDiagram
    ...
    User->>Manager: Start(ctx)
    Manager->>Cache: Start cache
    Manager->>Cache: Wait for cache sync
    
    alt Leader Election Enabled
        Manager->>LeaderElection: Start leader election
        LeaderElection->>Manager: Elected as leader
    end
    
    Manager->>Controllers: Start controllers
    Manager->>Webhooks: Start webhooks
    Manager->>Manager: Block until context cancelled
    
    User->>Manager: Cancel context
    Manager->>Controllers: Stop controllers
    Manager->>Webhooks: Stop webhooks
    Manager->>Cache: Stop cache
    Manager->>User: Return
```
...
stateDiagram-v2
    ...
    Leader --> Candidate: Lose Leadership
    Follower --> Candidate: Leader Lost
    Leader --> [*]: Shutdown
    Follower --> [*]: Shutdown
    
    state Leader {
        [*] --> StartControllers
        StartControllers --> RenewLease
        RenewLease --> RenewLease: Periodic Renewal
    }
    
    state Follower {
        [*] --> WaitForLeader
        WaitForLeader --> WaitForLeader: Monitor Leader
    }
```
...
graph TB
    Manager[Manager]
    Manager --> LeaderGroup[Leader Election Group]
    Manager --> NonLeaderGroup[Non-Leader Group]
    Manager --> WebhookGroup[Webhook Group]
    
    LeaderGroup --> Controller1[Controller 1]
    LeaderGroup --> Controller2[Controller 2]
    
    NonLeaderGroup --> Cache[Cache]
    NonLeaderGroup --> Metrics[Metrics Server]
    
    WebhookGroup --> Webhook1[Webhook 1]
    WebhookGroup --> Webhook2[Webhook 2]
    
    style Manager fill:#e1f5ff
    style LeaderGroup fill:#fff4e1
    style NonLeaderGroup fill:#f0f0f0
    style WebhookGroup fill:#e8f5e9
```
...
- [Controller Package](./02-controller.md) - Controllers managed by the Manager
- [Cache Package](./06-cache.md) - Cache provided by the Manager
- [Client Package](./05-client.md) - Client provided by the Manager
- [Webhook Package](./10-webhook.md) - Webhooks managed by the Manager
